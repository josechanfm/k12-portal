    ...
    <div class="bg-white relative shadow  p-5 rounded-lg overflow-x-auto">
      ...
      <a-form ref="modalRef" :model="distribute" name="From" layout="vertical" autocomplete="off" :rules="rules"
        ...
        <a-form-item :wrapper-col="{ offset: 12, span: 10 }">
          <a-button key="back" :href="route('admin.notices.index')">返回</a-button>
...
export default {
  ...
  data() {
    return {
      breadcrumb: [
        { label: "表格列表", url: route('admin.notices.index') },
        { label: this.notice.id ? '表格修改' : '表格新增', url: null }
      ],
      distribute:{},
      datenoticeat: "YYYY-MM-DD",
      recipientOptions:[
  ...
  methods: {
    ...
    onFinish(event) {
      ...
      this.$inertia.post(route("admin.notice.distribute"), this.distribute, {
        onSuccess: (page) => {
          // this.modal.data = {};

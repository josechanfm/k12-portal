<template>
  <AdminLayout title="生活習慣和態度" :breadcrumb="breadcrumb">
    ...
    <div class="flex gap-5">
      ...
      <a :href="route('director.pre.klass.habit.export', {klass: klass.id,term_id: selectedTermId,})" class="ant-btn ant-btn-primary">
        <DownloadOutlined /> 滙出
      </a>
    </div>
    <div class="py-5">
...
export default {
  ...
  methods: {
    ...
    saveScores() {
      ...
      // return true;
      this.$inertia.put(
        route("director.pre.klass.habits.update", this.klass.id),
        data,
        {
          onSuccess: (page) => {
            console.log(page);
    ...
    onImport(e){
        this.$inertia.post(route('director.pre.klass.habit.import',this.klass.id),{'importFile':e.target.files[0]},{
            onSuccess:(page)=>{

<template>
    <AdminLayout :title="klass.tag+'班升班預處理.'" :breadcrumb="breadcrumb ">
        <div class="py-5">
            <KlassSelector routePath="director.klass.promotes.index" :currentKlass="klass"/>
        </div>
       <div>
        升班至:
...
export default {
    ...
    methods: {
        confirmPromote(){
            ...
            this.$inertia.put(route('director.klass.promotes.update',this.klass.id), {klassStudents:data}, {
                onSuccess: (page) => {
                    console.log(page)
                },
                onError: (error) => {
        ...
        confirmBatchPromote(){
            if(this.batchPromoteTo==0){
                this.$inertia.get(route('director.klass.promotes.index',this.klass.id));
            }else{
                var data=[]
                this.students.forEach(student=>{
                    if(student.selected){
